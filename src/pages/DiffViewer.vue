...
function navigateToChange(direction: "prev" | "next") {
    ...
    if (currentChangeIndex.value === -1) {
        ...
        const currentPos = changePositions.value.findIndex(
            (p) => p.index === currentChangeIndex.value,
        );
        if (direction === "next") {
            newPosIndex =
                currentPos < changePositions.value.length - 1 ? currentPos + 1 : 0;
        } else {
            newPosIndex =
                currentPos > 0 ? currentPos - 1 : changePositions.value.length - 1;
        }
    }
    ...
    nextTick(() => {
        ...
        if (viewMode.value === "side-by-side") {
            ...
            if (leftElement && leftPanel.value) {
                ...
                const relativeTop =
                    elementRect.top - containerRect.top + leftPanel.value.scrollTop;
            ...
            if (rightElement && rightPanel.value) {
                ...
                const relativeTop =
                    elementRect.top - containerRect.top + rightPanel.value.scrollTop;
            ...
            if (element && unifiedPanel.value) {
                ...
                const relativeTop =
                    elementRect.top - containerRect.top + unifiedPanel.value.scrollTop;
...
<template>
    <ToolWrapper title="Diff 檢視器" icon="bi-file-diff" description="比對兩個文本的差異，支援語法高亮。適合比對設定檔、log 檔案等。">
        <div class="d-flex flex-column h-100">
            <!-- 頂部工具列 -->
            <div class="toolbar d-flex justify-content-between align-items-center mb-3 p-2 rounded" style="
        ...
        ">
                <div class="d-flex align-items-center gap-3 flex-wrap">
                    ...
                    <div class="d-flex align-items-center gap-2">
                        ...
                        <div class="btn-group btn-group-sm" role="group">
                            ...
                            <input type="radio" class="btn-check" v-model="viewMode" value="unified"
                                id="view-unified" />
                    ...
                    <div class="d-flex align-items-center gap-2">
                        ...
                        <div class="btn-group btn-group-sm" role="group">
                            <input type="radio" class="btn-check" v-model="diffStyle" value="vscode"
                                id="style-vscode" />
                            <label class="btn btn-outline-primary" for="style-vscode">
                                VSCode
                            </label>
                            <input type="radio" class="btn-check" v-model="diffStyle" value="github"
                                id="style-github" />
                ...
                <div class="d-flex align-items-center gap-2">
                    ...
                    <div v-if="totalChanges > 0" class="d-flex align-items-center gap-2 border-end pe-2">
                        <span class="small text-muted"> 變更: {{ totalChanges }} </span>
                        <div class="btn-group btn-group-sm" role="group">
                            <button class="btn btn-outline-secondary" @click="navigateToChange('prev')" title="上一個變更">
                                <i class="bi bi-chevron-up"></i>
                            </button>
                            <button class="btn btn-outline-secondary" @click="navigateToChange('next')" title="下一個變更">
                                <i class="bi bi-chevron-down"></i>
                            </button>
                        </div>
                    </div>

                    <button class="btn btn-sm btn-outline-danger" @click="clearAll" title="清除">
                    ...
                    <template #first>
                        <div ref="leftInputContainer" class="h-100 d-flex flex-column pe-2">
                            ...
                            <div class="flex-grow-1 min-h-0">
                                <HighlightedEditor v-model="originalText" placeholder="貼上原始文本..." :language="language"
                                    :enable-highlight="enableHighlight" @scroll="handleLeftInputScroll" />
                            </div>
                        </div>
                    </template>
                    <template #second>
                        <div ref="rightInputContainer" class="h-100 d-flex flex-column ps-2">
                            ...
                            <div class="flex-grow-1 min-h-0">
                                <HighlightedEditor v-model="modifiedText" placeholder="貼上修改後的文本..." :language="language"
                                    :enable-highlight="enableHighlight" @scroll="handleRightInputScroll" />
                            </div>
                        </div>
                    </template>
                </ResizableSplitPane>
            ...
            <div class="output-section flex-grow-1 min-h-0">
                ...
                <!-- Side-by-Side 視圖 -->
                <div v-if="viewMode === 'side-by-side'" class="diff-output side-by-side h-100"
                    ...
                    <div class="diff-container">
                        <div class="diff-panel left-panel">
                            ...
                            <div ref="leftPanel" class="diff-content" @scroll="handleLeftScroll">
                                <div v-for="(line, index) in sideBySideData.left" :key="'left-' + index" :class="[
                                    'diff-line',
                                    `diff-line-${line.type}`,
                                    { 'current-change': line.index === currentChangeIndex },
                                ]" :data-line-index="index" :data-change-index="line.type !== 'unchanged' ? line.index : undefined
                                    ">
                                    <span class="line-number">{{ line.lineNumber || "" }}</span>
                                    <pre class="line-content" v-html="line.content
                        ...
                        <div class="diff-panel right-panel">
                            ...
                            <div ref="rightPanel" class="diff-content" @scroll="handleRightScroll">
                                <div v-for="(line, index) in sideBySideData.right" :key="'right-' + index" :class="[
                                    'diff-line',
                                    `diff-line-${line.type}`,
                                    { 'current-change': line.index === currentChangeIndex },
                                ]" :data-line-index="index" :data-change-index="line.type !== 'unchanged' ? line.index : undefined
                ...
                <!-- Unified 視圖 -->
                <div v-else class="diff-output unified h-100" :class="`diff-style-${diffStyle}`">
                    <div class="diff-container">
                        <div ref="unifiedPanel" class="diff-content">
                            <div v-for="(line, index) in unifiedData" :key="'unified-' + index" :class="[
                                'diff-line',
                                `diff-line-${line.type}`,
                                { 'current-change': line.index === currentChangeIndex },
                            ]" :data-line-index="index" :data-change-index="line.type !== 'unchanged' ? line.index : undefined
